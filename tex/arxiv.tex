...
three groups of explicit infinite families, including the progressions arising
from \(p=3\) and \(d_1=2,5\).  We make no claim toward resolving the
Erdős--Straus conjecture and no priority claim for the underlying construction.
\end{abstract}
...
\begin{proof}
For both progressions, \(n\equiv1\pmod4\), \(3\nmid n\),
\(2\mid b\), and \(3\mid b+2\).  Theorem~\ref{thm:offset} applies with
\(p=3\) and \(d_1=2\).
\end{proof}
...
\begin{proof}
Each progression satisfies \(n\equiv1\pmod4\) and \(3\nmid n\).  Direct
factorisation of either \(n\) or \(x=(n+3)/4\) gives \(5\mid b\), while
\(3\mid b+5\).  Apply Theorem~\ref{thm:offset} with \(p=3\) and \(d_1=5\).
\end{proof}
...
\(k=1\).  The project builds with no \texttt{sorry}.  The theorem statement
includes the positive denominators and the rational unit-fraction equality;
the formalisation is therefore not merely a check of a polynomial identity.
...
specialisations.  No computational coverage, density statement, or progress
toward the full Erdős--Straus conjecture is asserted.

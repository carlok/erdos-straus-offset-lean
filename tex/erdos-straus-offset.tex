...
a fixed-divisor lifting theorem, an exact divisibility test for arbitrary
transport steps, and three groups of explicit infinite families, including the
progressions arising from \(p=3\) and \(d_1=2,5\).  We make no claim toward
resolving the Erdős--Straus conjecture; the underlying divisor split is
classical.
\end{abstract}
...
\section{An exact test for transport steps}
...
seed.  The following criterion answers that question without asserting that
\(4pd\) is always minimal.
...
For example, the criterion proves directly that \(T=12\) does not preserve the
seed \((p,d,n)=(3,2,5)\), whereas Theorem~\ref{thm:lifting} supplies the step
\(T=24\).  This negative check is included as a Lean regression.
...
construction, an exact transport-step criterion, and a small collection of
formally checked infinite specialisations.  The full Erdős--Straus conjecture
